...
// request overhead without retaining the whole film in mobile memory.
class FrameSheets {
  constructor(manifest, variant, framesBase, onLoad, onError) {
    ...
    this.prefetchSheets = Math.min(
      this.info.sheets,
      Math.ceil(PREFETCH_FRAMES / this.perSheet),
    );
    this.cache = new Map();
...
export class FramePlayer {
  constructor({ canvas, status, framesBase = "https://demos.riwaaz.in/rameswaram/public/assets/frames", poster = "https://demos.riwaaz.in/rameswaram/public/assets/poster/rameswaram-poster.jpg", onFrame, onReady, onError }) {
    this.canvas = canvas;
  ...
  async load() {
    try {
      ...
      this.variant = coarse && innerWidth < 420 ? "mobile" : "desktop";
      this.coarse = coarse;
      this.rate = coarse ? 1.6 : 2.2;
  ...
  drawPoster() {
    if (!this.poster || !this.poster.complete || !this.poster.naturalWidth) return;
    this.drawImage(
      this.poster,
      0,
      0,
  ...
  tick(now) {
    ...
    if (this.drawFrame(index)) this.time = next;
      ...
      if (now - this.waitSince > 400 && this.status)
